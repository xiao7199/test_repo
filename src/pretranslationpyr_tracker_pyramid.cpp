...
using PPTP = PretranslationPyrTrackerPyramid<M, C>;

template <class M, class C>
PretranslationPyrTrackerPyramid<M, C>::PretranslationPyrTrackerPyramid(AlgorithmParameters ap) :
  _levels(ap.levels)
{
  _tracker_pyr.clear();
  ...
  AlgorithmParameters tap = ap;
  tap.levels = 2;
  //tap.min_disp /= 2;
  for (int i = 0; i < _levels; i++) {
    _tracker_pyr.push_back(new Tracker<M, C>(ap));
    _pre_tracker_pyr.push_back(new SimpleTrackerPyramid<Translation, C>(tap));
...
template <class M, class C>
PretranslationPyrTrackerPyramid<M, C>::~PretranslationPyrTrackerPyramid() {
  for (int i = 0; i < _levels; i++) {
...
template <class M, class C>
void PretranslationPyrTrackerPyramid<M, C>::set_template(const Mat& I, const PointVec& pts,
...
template <class M, class C>
Result<M> PretranslationPyrTrackerPyramid<M, C>::step(const Mat& I, const Mat& mask,
                           const typename M::ParameterStruct& init_params) {
  return _tracker.step(I, mask, init_params);
}

template class PretranslationPyrTrackerPyramid<Affine, TwoBit>;
template class PretranslationPyrTrackerPyramid<Homography, TwoBit>;

template class PretranslationPyrTrackerPyramid<Affine, OneBit>;
template class PretranslationPyrTrackerPyramid<Homography, OneBit>;
